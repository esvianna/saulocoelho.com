...
| D37 | **Avaliação Final Neuropsicanálise** (#12): no **vtis-quiz** (não AmaEducacional); seed MC via `options_json` por pergunta (gabarito); `result_delivery` = **deferred**; timer 90 min; slug canónico **`neuropsicanalise`** (URL `/quiz/neuropsicanalise/`, legado `neuropsicanalise-final`). Prática + UI admin de gabarito **fora**. **Não** regravar o quiz no admin (apaga opções MC). |
