...
| D37 | **Avaliação Final Neuropsicanálise** (#12): no **vtis-quiz** (não AmaEducacional); seed `neuropsicanalise-final` com MC via `options_json` por pergunta (gabarito); `result_delivery` = **deferred**; prática + timer + UI admin de gabarito **fora** desta entrega. Não regravar o quiz no admin (apaga opções MC). |
