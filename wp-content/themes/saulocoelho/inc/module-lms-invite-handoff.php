 ...
 *
 * O plugin cria a conta, matricula e envia e-mail de acesso. Se o e-mail
 * não chega, o aluno fica sem senha e sem próximo passo. Este módulo:
 * 1) autentica na hora (cookie) após user_register em /inscricao/{slug}/
 * 2) leva o aluno à sala do curso quando a matrícula já existe
 * 3) mostra como definir senha em Minha Conta (não depende do e-mail)
 *
 * @package SauloCoelho
 */
...
function sc_invite_handoff_footer_cta() {
	...
	$strings = array(
		...
		'lead'       => __( 'Pode entrar na sala agora. Não precisa esperar o e-mail — ele às vezes cai no spam.', 'saulocoelho' ),
		'ctaCourse'  => __( 'Entrar na sala do curso', 'saulocoelho' ),
		'ctaPwd'     => __( 'Definir minha senha', 'saulocoelho' ),
		'ctaLost'    => __( 'Não recebi o e-mail — redefinir senha', 'saulocoelho' ),
	);
	?>
	<style>
		.sc-invite-handoff {
		...
		.sc-invite-handoff p { margin: 0 0 1rem; font-size: 0.95rem; line-height: 1.5; }
...
/**
 * Faixa na sala do curso (destino após o convite).
 */
function sc_invite_handoff_course_banner() {
	...
	<style>
		.sc-invite-handoff--hub { padding: 1.25rem 1.35rem; border-radius: 12px; border: 1px solid rgba(197, 160, 89, 0.4); background: rgba(197, 160, 89, 0.1); }
		.sc-invite-handoff--hub h2 { margin: 0 0 0.5rem; font-size: 1.15rem; color: #C5A059; }
		.sc-invite-handoff--hub p { margin: 0 0 1rem; }
