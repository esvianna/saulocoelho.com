...
function sc_presencial_admin_menu() {
	add_submenu_page(
		'woocommerce',
		__( 'Inscrições presenciais', 'saulocoelho' ),
		__( 'Inscrições presenciais', 'saulocoelho' ),
		'manage_options',
...
function sc_presencial_admin_page() {
	...
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Inscrições presenciais', 'saulocoelho' ); ?></h1>
...
function sc_presencial_export_csv() {
	...
	header( 'Content-Disposition: attachment; filename=inscricoes-presenciais.csv' );

	$out = fopen( 'php://output', 'w' );
	fputcsv( $out, array( 'ID', 'Aluno', 'E-mail', 'Produto', 'Pedido', 'Pagamento', 'Questionário', 'Presença', 'Data' ) );
